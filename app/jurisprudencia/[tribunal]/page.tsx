 ...
 *
 * SSG com revalidate 1h (decisões novas aparecem em até 1h sem rebuild).
 */
...
const TRIBUNAL_META: Record<TribunalSlug, { name: string; fullName: string; intro: string; fonte: string }> = {
  stf: {
    name: "STF",
...
export async function generateMetadata({ params }: { params: { tribunal: string } }) {
  const slug = params.tribunal.toLowerCase() as TribunalSlug;
  if (!VALID_TRIBUNALS.includes(slug)) {
    return buildMetadata({ title: "Jurisprudência", description: "Não encontrado", noIndex: true });
  }
...
export default async function TribunalPage({ params }: { params: { tribunal: string } }) {
  const slug = params.tribunal.toLowerCase() as TribunalSlug;
  ...
  return (
    <div className="container-narrow py-10">
      ...
      <section className="card mb-6">
        ...
        {recentes.length === 0 ? (
          <p className="text-sm text-brand-ink/65 italic">
            Sem decisões cadastradas ainda. O AdvAqui está em coleta inicial. Volte em breve.
          </p>
        ) : (
              ...
              <li key={d.id}>
                ...
                >
                  <div className="flex flex-wrap items-center gap-2 mb-1 text-xs">
                    {d.classe && <span className="font-semibold text-brand-deep">{d.classe} {d.numero}</span>}
                    {d.relator && <span className="text-brand-ink/55">— Rel. {d.relator}</span>}
                    {d.data_julgamento && (
                  ...
                  <p className="text-sm text-brand-ink/85 line-clamp-2">{d.ementa}</p>
                </Link>
              </li>
            ))}
          </ul>
        ...
        <div className="space-y-3">
          {FAQS_BY_TRIBUNAL[slug].map((f, i) => (
            ...
            >
              ...
              <p className="mt-2 text-sm text-brand-ink/80 leading-relaxed">{f.a}</p>
            </details>
          ))}
        </div>
      </section>
      ...
      >
        ...
        <span>
          Conteúdo extraído de fontes públicas. Para fins oficiais, consulte sempre a
          versão disponível na fonte original. Fonte:{" "}
          <a href={meta.fonte} target="_blank" rel="noopener noreferrer" className="underline">
            {meta.fonte}
          </a>
          .
        </span>
