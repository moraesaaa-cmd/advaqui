...
export async function POST(req: Request) {
  ...
  switch (action) {
    ...
    case "toggle-article-status": {
      ...
      const { error } = await supabase.from("blog_articles" as never).update(update).eq("id", body.id);
      if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
      revalidatePath("/blog");
    ...
    case "delete-article": {
      if (!body.id) return NextResponse.json({ ok: false, error: "ID ausente" }, { status: 400 });
      const supabase = createAdminClient();
      const { error } = await supabase.from("blog_articles" as never).delete().eq("id", body.id);
      if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
      revalidatePath("/blog");
      return NextResponse.json({ ok: true });
    }
